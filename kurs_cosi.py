...
class ImageProcessor(QWidget):
    ...
    def initUI(self):
        ...
        self.edgeButton = QPushButton("Выделение границ (ЛоГ)")
        self.edgeButton.clicked.connect(self.apply_log_operator)
        ...
        self.abutalebButton = QPushButton("Сегментация (Abutaleb)")
        self.abutalebButton.clicked.connect(self.abutaleb_segmentation)
        layout.addWidget(self.abutalebButton)
    ...
    def apply_log_operator(self):
        if self.image is not None:
            log_img = laplace(gaussian_filter(self.image, sigma=1))
    ...
    def abutaleb_segmentation(self):
        if self.image is not None:
